...
- "아홉 개의 문"은 각각 완전히 다른 방식(다이얼, 별자리, 저울, 카드, 문양 암기, 타이밍, 미로, 탐색, 문답)으로 열리며, 힌트는 페이지 내부의 "속삭임" 영역에서만 얻을 수 있습니다.
- 정답이 필요한 문(다이얼/문답)은 평문 비교 대신 체크섬 비교 방식을 사용해, 브라우저 개발자 도구로 소스를 열어도 정답이 그대로 노출되지 않도록 했습니다.
